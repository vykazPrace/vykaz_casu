...
<div id="container">
    ...
    <div class="Object1104"><span class="textstyle0">
            <h2><u>Vyhľadávanie projektov</u></h2>
    <br></br>
       <form action="pripojit_sa_do_projektu.php" method="post">
           <p>Vyhladaj: <input type="text" name="vyhladaj"/></p>
           <p><input type="submit" /></p>
       </form>
            <?php
            include "../prihlaseniedodatabazi.php";
            if (!isset($_GET['pripojit'])){
                if(isset($_POST['vyhladaj'])){
                    $vyhladaj = $_POST['vyhladaj'];



                    $co="SELECT * FROM projekt WHERE zamestnanec IS NULL AND nazov like '%$vyhladaj%'";
                    ...
                    $meno2=mysqli_query($con,"SELECT id_meno FROM uzivatelia WHERE meno='$meno'");
                    $meno3=mysqli_fetch_array($meno2);
                    $meno4=$meno3["id_meno"];
                    ...
                    $result=mysqli_query($con, "SELECT * FROM projekt WHERE zamestnanec IS NULL AND nazov like '%$vyhladaj%' ORDER BY nazov DESC LIMIT $pociatok, $limit") or die ("Spojenie s databázou bolo neúspešné");
                    $num_results=mysqli_num_rows($result);

                    echo "<table class='tabulka' border='0' width='600'>";
                    echo "<tr><th>Projekt</th><th>Zaciatok</th><th>Koniec</th><th>Veduci</th><th></th></tr>";
                    ...
                    {
                        $nazov2=$row["nazov"];
                        $kontrola=mysqli_query($con,"SELECT zamestnanec FROM projekt WHERE nazov='$nazov2' AND zamestnanec='$meno4'");
                        $kontrola2=mysqli_fetch_array($kontrola);
                        if(!isset($kontrola2["zamestnanec"])){
                            ...
                            $veduci=mysqli_query($con,"SELECT veduci FROM projekt WHERE nazov='$nazov2'");
                            $veduci2=mysqli_fetch_array($veduci);
                            echo "<tr><td><a href=\"index.php?ctrl=1&strana=",$strana,"&projekt=",$row["nazov"],"\"style=\"text-decoration:none;\">",$row["nazov"],"</a></td>";
                            echo "<td>",$start2["start_projektu"],"</td>";
                            echo "<td>",$end2["koniec_projektu"],"</td>";
                            echo "<td>",$veduci2["veduci"],"</td>";

                            echo "<td><a href=\"pripojit_sa_do_projektu.php?projekt=",$veduci2["veduci"],"&pripojit=",$row["nazov"],"\">","Pridat sa","</a></td></tr>";
                ...
                echo "Boli ste pridanydo projektu:<b><font color='red' >",$_GET['pripojit'],"</font></b>";
                $nazov=$_GET['pripojit'];
                $meno = $_SESSION['prihlaseny'];
                $meno2=mysqli_query($con,"SELECT id_meno FROM uzivatelia WHERE meno='$meno'");
                $meno3=mysqli_fetch_array($meno2);
                $meno4=$meno3["id_meno"];
                $veduci=mysqli_query($con,"SELECT veduci FROM projekt WHERE nazov='$nazov'");
                $veduci2=mysqli_fetch_array($veduci);
                $veduci3=$veduci2["veduci"];
                $start_projektu=mysqli_query($con,"SELECT start_projektu FROM projekt WHERE nazov='$nazov'");
                $start_projektu2=mysqli_fetch_array($start_projektu);
                $start_projektu3=$start_projektu2["start_projektu"];
                $koniec_projektu=mysqli_query($con,"SELECT koniec_projektu FROM projekt WHERE nazov='$nazov'");
                $koniec_projektu2=mysqli_fetch_array($koniec_projektu);
                $koniec_projektu3=$koniec_projektu2["koniec_projektu"];
                $popis=mysqli_query($con,"SELECT popis FROM projekt WHERE nazov='$nazov'");
                $popis2=mysqli_fetch_array($popis);
                $popis3=$popis2["popis"];


                $sql="INSERT INTO projekt(nazov,popis,veduci,zamestnanec,start_projektu,koniec_projektu) VALUES('$nazov','$popis3','$veduci3','$meno4','$start_projektu3','$koniec_projektu3' )";

...
<div class="Object1124"><span class="textstyle0">
         <h2><u>Úlohy</u></h2>
        <br></br>
        <a href="vytvor_ulohu_formular.php"><img src="files/IMG_160.jpg"  alt=""></a>
         <br></br>
        Úlohy na projekte:
        ...
        $id3=$id2['id_meno'];
        $co="SELECT * FROM projekt WHERE veduci='$id3' AND zamestnanec IS NULL";
        ...
        if(isset($_POST['select1'])){
            echo "<h2>Výpis úloh na projekte: <font color ='green'><u>",$_POST['select1'],"</u></font></h2>";
            ...
            $result=mysqli_query($con, "SELECT * FROM aktivita WHERE id_projekt='$id_proj3' ORDER BY meno") or die ("Spojenie s databázou bolo neúspešné");
            $num_results=mysqli_num_rows($result);
            echo "<table class='tabulka' border='0' width='600'>";
            echo "<tr><th>Aktivita</td><th>Popis</td><th>Pocet hodin</td><th>Zamestnanec</td><th></td><th></td><th></td><th></td></tr>";
            while ($row=mysqli_fetch_array($result))
            {
                ...
                $id_meno=mysqli_query($con,"SELECT id_meno FROM aktivita WHERE id_aktivita='$nazov2'");
        ...
        $id2=mysqli_fetch_array($id);
        $id3=$id2['id_meno'];
        $co2="SELECT zamestnanec FROM projekt WHERE nazov='$nazov_proj' AND zamestnanec IS NOT NULL";
        $result3=mysqli_query($con, $co2);
        $result4=mysqli_fetch_array($result3);
        $result5=$result4["zamestnanec"];
        ...
        {
            $zamestnanec=$row5["zamestnanec"];
            $co3="SELECT meno FROM uzivatelia WHERE id_meno='$zamestnanec' ";
            $result6=mysqli_query($con, $co3);
            $row6=mysqli_fetch_array($result6);

            echo "<option value=\"$zamestnanec\">", $row6["meno"],"</option>";

        }
        echo "<input value=\"Prihlasiť\" type=\"submit\" />";
        echo "</select>";
        echo "</form>";
        ...
        echo "</td>";
              ...
              $id_meno22=$id_meno2["id_meno"];
              $co4="SELECT meno FROM uzivatelia WHERE id_meno='$id_meno22' ";
              $result7=mysqli_query($con, $co4);
              $row6=mysqli_fetch_array($result7);
              echo "<td>",$row6["meno"],"</td>";
          }
                if($spravene2["stav"]==0){
                    echo "<td align=\"center\"><font color='red'><b>Nespravene</b></font></td>";
                }else{
                    echo "<td align=\"center\"><font color='green'><b>Spravene</b></font></td>";

                }
                echo "<td><img src=\"files/show.png\"></td>";
                echo "<td><img src=\"files/update.png\"   ></td>";
        ...
        if(isset($_POST['select2'])){
            ...
            $update=$_POST['select2'];
            $sql="UPDATE aktivita SET id_meno ='$update' WHERE id_aktivita='$id_ak'";

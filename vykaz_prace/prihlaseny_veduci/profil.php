...
<div id="container">
    ...
    <div class="Object1104"><span class="textstyle0">
        ...
        <br></br>
            ...
            $select = mysqli_query($con, "SELECT * FROM uzivatelia WHERE meno='$meno'") or die (mysql_error());
            $row=mysqli_fetch_array($select);
            echo "<table border='0' width='300'>";
            echo "<tr><td>Login: </td><td><strong>".$_SESSION['prihlaseny']."</strong></td></tr>";
            echo "<tr><td>Meno: </td><td><strong>".$row['meno_uzivatela']."</strong></td></tr>";
            echo "<tr><td>Priezvisko:</td><td><strong>".$row['priezvisko_uzivatela']."</strong></td></tr>";
            echo "<tr><td>Adresa: </td><td><strong>".$row['adresa']."</strong></td></tr>";
            ...
            <form action="profil.php" method="post">
                <table border="0">
                    <tr><td>Súčasné heslo:</td><td> <input type="password" name="heslo"/></td></tr>
                    <tr><td>Nové heslo:</td><td> <input type="password" name="noveheslo"/></td></tr>
                    <tr><td><input type="submit" value="Zmeniť" /></td></tr>
                </table>
            </form>
            <?php
            if(isset($_POST['heslo'])){
                include "../prihlaseniedodatabazi.php";
                $meno =  $_SESSION['prihlaseny'];
                $select = mysqli_query($con, "SELECT heslo FROM uzivatelia WHERE meno='$meno'") or die (mysql_error());
                $row=mysqli_fetch_array($select);
                $heslo = sha1($_POST['heslo']);
                $noveheslo = sha1($_POST['noveheslo']);
                if($row['heslo']==$heslo){
                    $sql=" UPDATE uzivatelia SET heslo='$noveheslo' WHERE meno='$meno'";
                    ;
                    ...
                    echo "Zadane heslo sa nezhoduje so sučasnim heslom";
                }
            }
            ?>
